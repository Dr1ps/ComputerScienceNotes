...
\[
\lim_{h \to 0} \frac{f(x_0 + h \cdot \alpha, y_0 + h \cdot \beta) - f(x_0,y_0)}{h}
\]
...
Ricordiamo il teorema di \textbf{Weierstrass} per le funzioni ad 1 variabile:\\
Se $f[a,b] \to \mathbb{R}$ è continuam allora $f$ ha sicuramente massimo e minimo.\\
Per trovare massimi e minimi in tali funzioni si controllano:
\begin{itemize}
  \item Punti stazionari interni (Fermat, derivata prima è 0)
  \item Punti singolari interni (ovvero per cui la derivata prima non esiste, come $x=0$ per $f(x) = |x|$).
  \item Punti di bordo, ovvero $a$ e $b$.
\end{itemize}
In una funzione a più variabili la cosa è la stessa e si analizzano:
\begin{itemize}
  \item Punti stazionari interni (come un punto "sella")(Le derivate parziali sono tutte 0).
  \item Punti singolari interni (almeno una delle derivate parziali non esiste)
  \item Punti di bordo, come quelli su una circonferenza (infiniti per $n \geq 2$)
\end{itemize}
\subsection{Geometria del vettore gradiente}
Consideriamo le derivate direzionali di una funzione $f$ come se fossero determinate da quelle fatte rispetto ai vettori di norma $1$:
\[
||v|| = 1
\]
Si può verificare infatti che:
\[
\frac{\delta f}{\delta (\lambda v)}(x_0) = \lambda \cdot \frac{\delta f}{\delta v}(x_0)
\]
E quindi possiamo lavorare con norma $1$.\\
Se $f$ è differenziabile in $x_0$, per quale direzione $v$ con $||v|| = 1$ si verifica che la derivata direzionale assume il valore massimo o minimo?\\
\begin{itemize}
  \item \textbf{Massimo:} nella direzione del gradiente:
  \[
  \frac{\delta f}{\delta \nabla f(x_0)}(x_0)
  \]
  \item \textbf{Minimo:} nella direzione opposta del gradiente:
  \[
  \frac{\delta f}{\delta -\nabla f(x_0)}(x_0)
  \]
\end{itemize}
Questo si dimostra con la formula del prodotto scalare:
\[
  \frac{\delta f}{\delta v}(x_0) = \nabla f(x_0) \cdot v = ||\nabla f(x_0)|| \cdot ||v|| \cdot cos\Theta = \ldots
\]
\[
\ldots = ||\nabla f(x_0)|| \cdot cos\Theta
\]
Questo raggiunge il massimo quando $cos\Theta = 1$, e quindi $\Theta = 0$, il minimo invece analogamente ($\Theta = \frac{\pi}{2}$).\\
Se invece il tutto è $= 0$ allora la direzione è tangente alla curva di livellodi $f$ in $x_0$.
\paragraph{Esempio:}
\[
f(x,y) = x^2 + y^2
\]
Quale è la curva di livello per cui:
\[
f(x,y) = \lambda
\]
\[
x^2 + y^2 = \lambda
\]
In questo caso $\lambda \geq 0$.\\
La curva di livello è una circonferenza centrata in $(0,0)$ con raggio $\sqrt{\lambda}$.\\
Nel piano tridimensionale corrisponde ad un grafico che sembra un vaso che cresce sempre di piu.\\
Il vettore per cui questo grafico cresce di più come sappiamo è il vettore gradiente:
\[
\nabla f(2x,2y)
\]
Dato che:
...
\]
\subsection{Campo di vettori}
Il vettore gradiente va da $\mathbb{R}^n$ a $\mathbb{R}^n$ e associa ad ogni punto del grafico un vettore.
\subsection{Piano Tangente}
Ricordiamo che in 1 variabile se $f$ è differenziabile allora:
\[
f(x_0 + h) = f(x_0) + f^|(x_0) \cdot h + o(h)
\]
Sostituiamo $x_0 + h$ con $x$.
\[
f(x) = f(x_0) + f^|(x_0) \cdot(x-x_0) + (x-x_0)
\]
Questa è proprio la formula della retta tangente, per un piano invece:
\[
v = (h,k)
\]
\[
f(x_0 + h, y_0 + k) = f(x_0,y_0) + (\frac{\delta f}{\delta x}(x_0,y_0) + \frac{\delta f}{\delta y}(x_0,y_0)) \cdot k + o(||(h,k)||)
\]
\[
f(x, y) = f(x_0,y_0) + (\frac{\delta f}{\delta x}(x_0,y_0))\cdot (x-x_0) + (\frac{\delta f}{\delta y}(x_0,y_0)) \cdot (y-y_0) + o(||(h,k)||)
\]
Questa è l'equazione del piano tangente al grafico in $(x_0,y_0,y(x_0,y_0))$
\paragraph{Esempio:}
\[
f(x,y) = x^2 \cdot \cos(y)
\]
Piano tangente in $(x_0,y_0) = (1,1)$
\[
f(x_0,y_0) = f(1,1) = \cos(1)
\]
\[
\frac{\delta f}{\delta x} = 2x \cdot \cos(y)
\]
\[
\frac{\delta f}{\delta y} = x^2 \cdot (- \sin(y))
\]
\[
\nabla f (2x \cdot \cos(y), x^2 \cdot -(\sin(y)))
\]
\[
\frac{\delta f}{\delta x}(1,1) = 2 \cdot \cos(y)
\]
\[
\frac{\delta f}{\delta y}(1,1) = -\sin(1)
\]
\[
z = cos(1) + 2\cos(1)(x-1) - sin(1)(y-1)
\]
Nella formula sopra:
\begin{itemize}
  \item $\cos(1) =$ $f(x_0,y_0)$
  \item $2cos(1) = \frac{\delta f}{\delta x}(x_0,y_0)$
  \item $x-1 = x_0$
  \item $-\sin(1) = \frac{\delta f}{\delta y}(x_0,y_0)$
  \item $y-1 = y_0$
\end{itemize}
\subsection{Teorema del differenziabile totale}
Se $f$ ha tutte le derivate parziali definite in un intorno di $x_0$ e sono continue in $x_0$ allora $f$ è differenziabile in $x_0$.\\
Nella pratica, per verificare che una funzione data sia differenziabile, bisogna:
\begin{itemize}
  \item Calcolare le derivate parziali
  \item Verificare che siano definite in un intorno di $x_0$
  \item Verificare che siano continue (se sono composte solo da funzioni elementari / solite allora sono continue)
\end{itemize}
